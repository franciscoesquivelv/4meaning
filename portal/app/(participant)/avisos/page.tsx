...
function tipoStyle(tipo: string) {
  const map: Record<string, { bar: string; badge: string; label: string }> = {
    ...
    logistica: { bar: 'border-l-terra', badge: 'bg-terra/10 text-terra', label: 'Logística' },
  }
  return map[tipo] ?? { bar: 'border-l-line', badge: 'bg-paper-2 text-gray', label: tipo }
}
...
export default async function AvisosParticipantPage() {
  ...
  return (
    <div className="px-5 pt-6 pb-4">
      ...
      <p className="text-sm text-gray mb-6">Comunicados del equipo de Trascendencia</p>

      {!family && (
        <div className="text-center py-16 text-gray text-sm">
      ...
      {family && announcements.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <div className="w-12 h-12 rounded-full bg-white border border-line flex items-center justify-center mb-4 text-2xl text-gray">
            📢
          </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs text-gray">{formatDate(a.created_at)}</span>
                </div>
                <p className="font-semibold text-ink text-sm leading-snug">{a.titulo}</p>
                {a.cuerpo && (
                  <p className="mt-2 text-sm text-gray leading-relaxed whitespace-pre-wrap">{a.cuerpo}</p>
                )}
              </div>
            )
          })}
