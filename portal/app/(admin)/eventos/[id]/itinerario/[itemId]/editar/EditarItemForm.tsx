...
export default function EditarItemForm({ item, teamMembers }: Props) {
  ...
  const isCustomResponsable =
    item.responsable !== null &&
    item.responsable !== '' &&
    !teamMembers.some(m => m.nombre === item.responsable)
  ...
  const [form, setForm] = useState({
    ...
    dia: String(item.dia),
    fecha: item.fecha ?? '',
    hora_inicio: item.hora_inicio ? item.hora_inicio.slice(0, 5) : '',
    hora_fin: item.hora_fin ? item.hora_fin.slice(0, 5) : '',
    tipo: item.tipo,
    descripcion: item.descripcion ?? '',
    ubicacion: item.ubicacion ?? '',
    responsable: isCustomResponsable ? '__custom__' : (item.responsable ?? ''),
    responsable_custom: isCustomResponsable ? (item.responsable ?? '') : '',
    visibilidad: item.visibilidad,
    notas_staff: item.notas_staff ?? '',
    grupo: item.grupo ?? '',
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    router.push(`/eventos/${item.event_id}/itinerario`)
  }
  ...
  return (
    <div className="p-8 max-w-2xl">
      <div className="mb-6">
        <Link
          href={`/eventos/${item.event_id}/itinerario`}
          className="text-sm text-slate-500 hover:text-slate-700 transition-colors"
        >
          ← Itinerario
        </Link>
        <h1 className="text-2xl font-bold text-slate-900 mt-3">Editar item</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <input
            ...
            onChange={set('grupo')}
            className={inputCls}
            placeholder="Todos / Grupo A / Grupo B"
          />
        </div>
