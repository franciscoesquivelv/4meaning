...
import { bloquesDe, tieneContenido } from '../../../personalab/contenido'
import BloqueLector from '../../../personalab/Bloques'
      ...
      <article className="px-6 md:px-0 pb-24">
        <div className="max-w-[620px] mx-auto">
          ...
          <footer className="mt-[88px] md:mt-[120px] pt-8 border-t border-[#E7E1D8]">
            {siguiente ? (
              <Link href={`/prototipo/lector/${c.id}/${siguiente.id}${q}`} className="group block">
                {cambiaTiempo && (
            ...
            {anterior && (
              <div className="mt-10">
                <Link
                  href={`/prototipo/lector/${c.id}/${anterior.id}${q}`}
                  className="text-[12.5px] font-light text-[#676E6E] hover:text-[#002B34] transition-colors"
                >
                  ← {anterior.titulo}
                </Link>
