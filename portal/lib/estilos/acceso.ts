...
// Fondo del umbral. Sin clase de marca: el umbral lo cruza cualquier cuenta
// (Trascendencia, PersonaLab, equipo), así que no puede llevar el color de
// una sub-marca. `--dom`/`--dom-deep` sin ninguna clase `marca-*` resuelven
// a su valor de :root, que es el teal ("Verde Petróleo · color rector de la
// casa", marca.css), no un color inventado para esta pantalla. Trascendencia
// se aparta de ese default con su propia clase en su propio árbol; aquí, en
// terreno de la casa, no hay apartamiento.
//
// Antes llevaba `marca-trascendencia` a mano, heredado de cuando este umbral
// era solo de Trascendencia: pintaba vino profundo para cualquier cuenta,
// incluidas las de PersonaLab. Corregido 2026-09-21.
export const LIENZO =
  "min-h-screen bg-dom-deep flex flex-col items-center justify-center px-6 py-12"

// Tarjeta elevada. Teal sobre teal profundo: la misma elevacion de dos
// tonos que el sitio usa entre el hero y el bloque de las tres preguntas,
// ahora con el color rector de la casa y no el de una sub-marca.
export const TARJETA =
  "w-full max-w-sm bg-dom border border-line-dk rounded-2xl p-8"

// Etiqueta de campo. Papel atenuado, que sobre la tarjeta (teal) da 7.07 a 1.
// Re-medido 2026-09-21 al quitar `marca-trascendencia` del umbral: antes
// decia 7.3, medido sobre vino.
export const ETIQUETA = "block text-xs text-paper/70 mb-1.5"

// Campo. Las cifras estan medidas en el navegador, sobre la pantalla
// corriendo, no calculadas de la paleta. Re-medidas 2026-09-21 sobre teal,
// el color por defecto del umbral desde que se le quito `marca-trascendencia`
// (antes eran sobre vino; se anota el numero anterior donde cambio):
...
//     fondo (1.19 contra la tarjeta): o sea que no se distinguia por nada.
//     En papel al 45% da 3.75 (antes 3.06 sobre vino) y es neutro, que es lo
//     que un campo en reposo debe ser.
//   - El foco entra en terracota clara SOLIDA, 7.35 (antes 7.39). Sube desde
//     3.75, que es la direccion correcta. Con la terracota de marca bajaba,
//     porque es mas oscura que la clara sobre este fondo.
//   - El placeholder da 7.09 contra el fondo del propio campo y lleva
//     informacion util (el formato del dato); al 65% no se confunde con un
//     valor ya escrito.
//
...
// teal, el mismo teal que hoy es el default del umbral): `alerta` (#8C2A22)
// como filete da 1.77 contra la tarjeta y 2.11 contra el fondo del campo.
// Los dos reprueban el 3:1 que pide un limite de control (WCAG 1.4.11), y de
// lejos: es la misma razon medida por la que ERROR, mas abajo, usa `alerta`
// como RELLENO solido con texto `paper` (7.39) y nunca como texto o filete
// suelto. Aqui se confirma que tampoco sirve como borde, ni en vino ni en teal.
//
...
// Accion principal. Sobre el fondo profundo el primario no puede ser el
// mismo tono, asi que se invierte a papel. Contraste medido: 15.61 a 1
// (antes 15.96 sobre vino; re-medido 2026-09-21 sobre teal). La terracota se
// reserva para el acento, que es su dosis en el sistema de marca.
export const BOTON =
...
// Error. Enmarcado, no texto rojo suelto: sobre el fondo oscuro del umbral
// ningun rojo de la paleta es legible como texto, y el vino de alerta si lo
// es como fondo solido (independiente del teal o vino de alrededor).
// Contraste medido: 7.39 a 1.
export const ERROR = "rounded-xl bg-alerta px-4 py-3 text-sm text-paper"

// Confirmacion. El verde semantico daba 2.91 a 1 sobre vino profundo, o sea
// que era ilegible ahi: sobre oscuro la senal buena es la terracota clara.
// Contra la tarjeta (donde vive este texto) da 7.35 a 1, re-medido 2026-09-21
// sobre teal.
export const CONFIRMACION = "text-sm text-terra-lo"

// Enlace secundario del pie de la tarjeta. Mismo color y misma tarjeta que
// CONFIRMACION: 7.35 a 1.
export const ENLACE =
